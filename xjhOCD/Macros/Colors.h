//
//  Colors.h
//  ZNZNew
//
//  Created by xjhuang on 2018/7/11.
//  Copyright © 2018年 xjhuang. All rights reserved.
//

#ifndef Colors_h
#define Colors_h

#define HexColor(__string)  [UIColor colorWithHexString:__string]
#define kColor_Main         [UIColor colorWithHexString:@"#1892FA"]
#define kColor_Main_New     [UIColor colorWithHexString:@"#65AE75"]
#define kColor_Disabled     kColor_Line
#define kColor_Gray         [UIColor colorWithHexString:@"#999999"]
#define kColor_BG           [UIColor colorWithHexString:@"#F0F0F0"]
#define kColor_Line         [UIColor colorWithHexString:@"#E6E7EC"]
#define kColor_Red          [UIColor colorWithHexString:@"#FF0000"]
#define kColor_Green        [UIColor colorWithHexString:@"#00FF00"]
#define kColor_Black        [UIColor colorWithHexString:@"#333333"]
#endif /* Colors_h */
